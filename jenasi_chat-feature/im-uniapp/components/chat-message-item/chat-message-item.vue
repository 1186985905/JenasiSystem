    ...
    <view class="chat-msg-normal" v-if="isNormal" :class="{ 'chat-msg-mine': msgInfo.selfSend }">
      ...
      <view class="chat-msg-content">
        ...
        <view class="chat-msg-bottom">
          <!-- 流式消息展示区域 -->
          <template v-if="msgInfo.type === $enums.MESSAGE_TYPE.TEXT">
            <div class="streaming-container">
              <div v-for="chunk in messageChunks" :key="chunk.id" class="streaming-message">
                <long-press-menu :items="menuItems" @select="onSelectMenu">
                  <rich-text
                      class="chat-msg-text"
                      :nodes="$emo.transform(chunk.content, 'emoji-normal')"
                  ></rich-text>
                </long-press-menu>
                <span v-if="isStreaming && chunk.id === currentStreamId" class="streaming-cursor">|</span>
              </div>
            </div>
          </template>
...
export default {
  ...
  data() {
    return {
      ...
      menu: { show: false, style: "" },
      // 新增流式消息相关状态
      messageChunks: [],      // 消息块数组
      isStreaming: false,     // 是否正在流式传输
      currentStreamId: null   // 当前流式消息ID
    };
  },
  mounted() {
    this.initWebSocket();
  ...
  methods: {
    // 初始化WebSocket连接
    initWebSocket() {
      const wsUrl = 'ws://your-im-server/ws'; // 替换实际WS地址
      this.socket = new WebSocket(wsUrl);

      this.socket.onmessage = (event) => {
        const data = JSON.parse(event.data);
        if (data.cmd === this.$enums.IMCmdType.STREAM_MESSAGE) {
          this.handleStreamChunk(data.data);
        }
      };
    ...
    handleStreamChunk(chunk) {
      ...
      } else if (chunk === '[END_STREAM]') {
        this.isStreaming = false;
        this.currentStreamId = null;
      } else {
        const index = this.messageChunks.findIndex(
            msg => msg.id === this.currentStreamId
        );
        if (index !== -1) {
          this.messageChunks[index].content += chunk;
          // 触发视图更新
          this.$forceUpdate();
          // 自动滚动到底部
          this.$nextTick(() => {
            const container = this.$el.querySelector('.chat-msg-content');
            if (container) {
