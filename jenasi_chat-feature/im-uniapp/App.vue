...
export default {
	...
	methods: {
		...
		initWebSocket() {
          ...
          if(msgInfo.type === 20){ // STREAM_START
            this.handlePrivateMessage(msgInfo);
          } else if(msgInfo.type === 21){ // STREAM_CHUNK
            this.chatStore.updateMessage({
              id: msgInfo.streamId,
              content: prev => prev + msgInfo.content
            });
          } else if(msgInfo.type === 22){ // STREAM_END
            this.chatStore.updateMessage({
              id: msgInfo.streamId,
              isStreaming: false
            });
      ...
      // 新增：流式消息初始化
      if (msg.type === enums.MESSAGE_TYPE.STREAM_START) {
        const tempMsg = {
          id: msg.streamId, // 流式消息唯一ID（需后端生成）
          text: "",
          isStreaming: true,
          type: enums.MESSAGE_TYPE.STREAM_CHUNK
        };
        this.chatStore.insertMessage(tempMsg, chatInfo);
        return;
      }
      ...
      if (msg.type === enums.MESSAGE_TYPE.STREAM_CHUNK) {
        // 更新流式消息内容
        this.chatStore.updateStreamMessage({
          streamId: msg.streamId,
          content: msg.content,
          isEnd: msg.isEnd // 是否结束标志
        });
        if (msg.isEnd) {
          this.playAudioTip(); // 播放完成提示音
        }
        return;
      }
...
<!--// 修改消息组件支持流式样式-->
<template>
  <div class="message" :class="{ 'streaming': message.isStreaming }">
    {{ message.text }}
