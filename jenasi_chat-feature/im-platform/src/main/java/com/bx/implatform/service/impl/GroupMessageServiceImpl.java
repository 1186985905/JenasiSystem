...
public class GroupMessageServiceImpl extends ServiceImpl<GroupMessageMapper, GroupMessage> implements
    ...
    @Resource
    PrivateMessageServiceImpl privatePrivateMessageService;   // 只获取AI的回复


    // 发送群聊消息  群聊id  发送内容  消息类型  被@用户列表
    @Override
    public GroupMessageVO sendMessage(GroupMessageDTO dto) {
        ...
        // 不用发给自己
        userIds = userIds.stream().filter(id -> !session.getUserId().equals(id)).collect(Collectors.toList());  // 去掉自己
        // 保存消息
        ...
        List<Long> atUserIds = dto.getAtUserIds();  // @用户列表

        if (atUserIds ==  null)   return msgInfo;   // 没有@用户列表，直接返回

        for(Long l : atUserIds){
            if (l == 1L){
                //改成异步
                CompletableFuture.runAsync(() -> sendAIMessage(dto));  // 异步调用机器让群聊回复

//                sendAIMessage(dto);  //调用机器让群聊回复
            }
            break;
        }
    ...
    // 发送ai群聊消息
    public GroupMessageVO sendAIMessage(GroupMessageDTO dto) {

        String AIontent = AIMsg.getAIMsg(dto.getContent());  //获取ai回复
        dto.setContent(AIontent);  // 设置ai回复的内容

//        UserSession session = SessionContext.getSession();
        Group group = groupService.getAndCheckById(dto.getGroupId());
        // 是否在群聊里面
        GroupMember member = groupMemberService.findByGroupAndUserId(dto.getGroupId(), 1L);
        if (Objects.isNull(member) || member.getQuit()) {
            throw new GlobalException("您已不在群聊里面，无法发送消息");
        }
        ...
        List<Long> userIds = groupMemberService.findUserIdsByGroupId(group.getId());
        // 不用发给自己
        userIds = userIds.stream().filter(id -> !id.equals(1L)).collect(Collectors.toList());
        // 保存消息
        GroupMessage msg = BeanUtils.copyProperties(dto, GroupMessage.class);
        msg.setSendId(1L);
        msg.setSendTime(new Date());
        msg.setSendNickName(member.getShowNickName());
        msg.setAtUserIds("");   //不at任何人
        // 过滤内容中的敏感词
        if(MessageType.TEXT.code().equals(dto.getType())){
            msg.setContent(sensitiveFilterUtil.filter(dto.getContent()));
        }
        ...
        msgInfo.setAtUserIds(dto.getAtUserIds());
        IMGroupMessage<GroupMessageVO> sendMessage = new IMGroupMessage<>();
        sendMessage.setSender(new IMUserInfo(1L, 0));
        sendMessage.setRecvIds(userIds);
        sendMessage.setSendResult(false);
        sendMessage.setData(msgInfo);
        imClient.sendGroupMessage(sendMessage);
        log.info("发送群聊消息，发送id:{},群聊id:{},内容:{}", 1L, dto.getGroupId(), dto.getContent());
        return msgInfo;
    }
